...
if($this->parameter('create')){	
	...
	elseif($this->error('username')){
		echo $this->color('red') . '[error]' . $this->color('end') . ' ' . 'A username cannot contain a .' . "\n";
	}
	elseif($this->error('user-exists')){
		echo $this->color('red') . '[error]' . $this->color('end') . ' ' . 'A user with this username already exists.' . "\n";
	} else {	
		echo 'config' ."\n";	
	}
}
